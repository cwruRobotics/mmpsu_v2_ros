...
class MmpsuV2Uart(MmpsuV2Base):
    ...
    def test_comms(self) -> bool:
        ...
        packet[0] = START_BYTE
        packet[CMD_IND] = MmpsuV2Commands.CMD_TEST_COMMS
        packet[HDDR_SIZE] = 0xDE
        packet[HDDR_SIZE + 1] = 0xAD
        packet[HDDR_SIZE + 2] = 0xBE
        packet[HDDR_SIZE + 3] = 0xEF
        ...
        with self._uart_lock:
            ...
            if rpy[HDDR_SIZE: HDDR_SIZE + get_payload_size(rpy)] != packet[HDDR_SIZE: HDDR_SIZE + get_payload_size(packet)]:
                return False
    ...
    def _verify_packet(self, tx_packet: bytearray, rx_packet: bytearray) -> bool:
        ...
        if len(rx_packet) < HDDR_SIZE + CRC_SIZE:
