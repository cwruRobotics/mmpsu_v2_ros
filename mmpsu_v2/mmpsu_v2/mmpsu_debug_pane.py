...
class MmpsuDebugPane(Node):
    ...
    def __init__(self):
        ...
        self.core_telem_sub = self.create_subscription(MmpsuCoreTelem, 'mmpsu/mmpsu_core', self.core_telem_callback, 10)
        self.aux_telem_sub = self.create_subscription(MmpsuAuxTelem, 'mmpsu/mmpsu_aux', self.aux_telem_callback, 10)
    ...
    def setup_widgets(self):
        ...
        self.output_state_btn = tk.Button(self.ctrl_frame, textvariable=self.output_state_strvar, command=self.toggle_output_state)
        ...
        tk.Button(self.ctrl_frame, text="SET", command=self.set_vout_setpt).grid(row=grid_row, column=2)
        grid_row += 1
